...
shared ({ caller = creator }) actor class () {
  ...
  server.get(
    "/get_row_db", func(req, res) : Response {
      ...
      var rowJson = "{ ";
      for (row in db.vals()) {
        rowJson := rowJson # "\"" # Nat.toText(counter) # "\": { \"id\": \"" # Nat.toText(row.id) # "\", \"companyName\": \"" # row.companyName # "\", \"cityDestination\": \"" # row.cityDestination # "\", \"supplier\": \"" # row.supplier # "\", \"cityOrigin\": \"" # row.cityOrigin # "\", \"productType\": \"" # row.productType # "\", \"quantity\": " # Nat.toText(row.qty) # " }, ";
        counter += 1;
      };
